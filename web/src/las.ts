...
export async function loadLAS(
  file: File, maxPoints: number, onProgress?: (pct: number) => void,
): Promise<PointData> {
  const h = new Uint8Array(await file.slice(0, 400).arrayBuffer())
  if (String.fromCharCode(h[0],h[1],h[2],h[3]) !== 'LASF') throw new Error('Not a valid LAS file')

  const vMaj=h[24], vMin=h[25], offPts=rU32(h,96), fmt=h[104], recLen=rU16(h,105)
  let numPts: number
  if (vMaj===1&&vMin>=4) { const lo=rU32(h,247),hi=rU32(h,251); numPts=hi>0?lo+hi*4294967296:(rU32(h,107)||lo) }
  else numPts=rU32(h,107)

  const xs=rF64(h,131),ys=rF64(h,139),zs=rF64(h,147)
  const xo=rF64(h,155),yo=rF64(h,163),zo=rF64(h,171)
  const xMin=rF64(h,187),yMin=rF64(h,203),zMin=rF64(h,219)
  const xMax=rF64(h,179),yMax=rF64(h,195),zMax=rF64(h,211)
  const rgbOff=RGB_OFF[fmt], hasRGB=rgbOff!==undefined
  ...
  if (hasRGB) {
    ...
    let mx=1
    for(let i=0;i<Math.min(2000,sb.length/recLen);i++){const o=i*recLen+rgbOff;if(o+6>sb.length)break;const r=rU16(sb,o),g=rU16(sb,o+2),b=rU16(sb,o+4);if(r>mx)mx=r;if(g>mx)mx=g;if(b>mx)mx=b}
    cScale=255/mx
  }
  ...
  const BLOCK=32*1024*1024, bPts=Math.floor(BLOCK/recLen), bBytes=bPts*recLen
  const dataSize=numPts*recLen
  let nextIdx=0

  for(let bStart=0;bStart<dataSize;bStart+=bBytes){
    const bEnd=Math.min(bStart+bBytes,dataSize), iStart=bStart/recLen, iEnd=bEnd/recLen
    if(nextIdx>=iEnd) continue
    ...
    while(nextIdx<iEnd&&nextIdx<numPts){
      ...
      if(lo<0||lo+recLen>buf.length) break
      const j=total
      ...
      if(hasRGB){const ro=lo+rgbOff;const r=(rU16(buf,ro)*cScale+.5)|0,g=(rU16(buf,ro+2)*cScale+.5)|0,b=(rU16(buf,ro+4)*cScale+.5)|0;colors[j*3]=r;colors[j*3+1]=g;colors[j*3+2]=b;if(total<500)grayDiff+=Math.abs(r-g)+Math.abs(g-b)}
      total++;nextIdx+=step
    }
  ...
  for(let i=0;i<total;i+=bStep){const x=pos[i*3],y=pos[i*3+1],z=pos[i*3+2];if(x<bxn)bxn=x;if(x>bxx)bxx=x;if(y<byn)byn=y;if(y>byx)byx=y;if(z<bzn)bzn=z;if(z>bzx)bzx=z}
  ...
  return {
    positions:positions.subarray(0,total*3), colors:colors.subarray(0,total*3),
    intensity:intensity.subarray(0,total), count:total,
...
function isDeletedByOp(px:number,py:number,pz:number, m:number[], L:number,T:number,R:number,B:number, vpW:number,vpH:number, keep:boolean): boolean {
  ...
  if(cw<=0) return keep
  const sx=(m[0]*px+m[4]*py+m[8]*pz+m[12])/(cw*2)+.5
  const sy=.5-(m[1]*px+m[5]*py+m[9]*pz+m[13])/(cw*2)
...
export function applyOp(data: PointData, op: EditOp): { result: PointData; removedCount: number } {
  ...
  if(removed===0) return {result:data,removedCount:0}

  const n=count-removed
  const np=new Float32Array(n*3), nc=new Uint8Array(n*3), ni=new Uint8Array(n)
  let j=0
  for(let i=0;i<count;i++){
...
export async function saveLAS(file: File, ops: EditOp[], onProgress: (pct:number)=>void): Promise<Blob> {
  if(!ops.length) return file

  const h=new Uint8Array(await file.slice(0,400).arrayBuffer())
  const vMaj=h[24],vMin=h[25],offPts=rU32(h,96),recLen=rU16(h,105)
  let numPts:number
  if(vMaj===1&&vMin>=4){const lo=rU32(h,247),hi=rU32(h,251);numPts=hi>0?lo+hi*4294967296:(rU32(h,107)||lo)}else numPts=rU32(h,107)
  const xs=rF64(h,131),ys=rF64(h,139),zs=rF64(h,147),xo=rF64(h,155),yo=rF64(h,163),zo=rF64(h,171)
  const xMin=rF64(h,187),yMin=rF64(h,203),zMin=rF64(h,219),xMax=rF64(h,179),yMax=rF64(h,195),zMax=rF64(h,211)
  const cx=(xMin+xMax)/2,cy=(yMin+yMax)/2,cz=(zMin+zMax)/2

  // Pass 1: mark deleted
  const deleted=new Uint8Array(numPts)
  const BLOCK=32*1024*1024,bPts=Math.floor(BLOCK/recLen),bBytes=bPts*recLen,dataSize=numPts*recLen
  for(let bStart=0;bStart<dataSize;bStart+=bBytes){
    const bEnd=Math.min(bStart+bBytes,dataSize),iStart=bStart/recLen,iEnd=Math.min(bEnd/recLen,numPts)
    const buf=new Uint8Array(await file.slice(offPts+bStart,offPts+bEnd).arrayBuffer())
    for(let i=iStart;i<iEnd;i++){
      const lo=(i-iStart)*recLen
      const px=rI32(buf,lo)*xs+xo-cx,py=rI32(buf,lo+4)*ys+yo-cy,pz=rI32(buf,lo+8)*zs+zo-cz
      for(const op of ops){
    ...
    onProgress(.4*bEnd/dataSize); await new Promise(r=>setTimeout(r,0))
  }

  let keepCount=0;for(let i=0;i<numPts;i++)if(!deleted[i])keepCount++

  // Write header
  const headerBuf=new Uint8Array(await file.slice(0,offPts).arrayBuffer())
  const outH=new Uint8Array(headerBuf),hDv=new DataView(outH.buffer)
  if(vMaj===1&&vMin>=4){hDv.setUint32(107,Math.min(keepCount,0xFFFFFFFF),true);hDv.setUint32(247,keepCount>>>0,true);hDv.setUint32(251,0,true)}else hDv.setUint32(107,keepCount,true)
  ...
  for(let bStart=0;bStart<dataSize;bStart+=bBytes){
    ...
    let blockKeep=0
    for(let i=iStart;i<iEnd;i++) if(!deleted[i]) blockKeep++
    if(blockKeep===0){onProgress(.4+.6*bEnd/dataSize);await new Promise(r=>setTimeout(r,0));continue}
    ...
    parts.push(out)
    onProgress(.4+.6*bEnd/dataSize); await new Promise(r=>setTimeout(r,0))
  }
